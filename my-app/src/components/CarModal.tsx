    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl relative flex flex-col md:flex-row overflow-hidden max-h-[90vh] overflow-y-auto">
        ...
        <div className="md:w-1/2 w-full p-6 flex flex-col justify-center space-y-3">
          ...
          <p><strong>Nombre Dueño:</strong> {car.owner ?? 'xxxxxxxxxxxxx'}</p>
          <p><strong>Marca Vehículo:</strong> {car.brand}</p>
          <p><strong>Modelo:</strong> {car.model}</p>
          <p><strong>Color:</strong> {car.color ?? 'xxxxxxxxxxxxx'}</p>
          <p><strong>Veces Alquilado:</strong> {car.totalRentals ?? 'XXXX'}</p>

          <div className="mt-4">
            <h4 className="text-lg font-semibold">Estado del automóvil:</h4>
            <p className="text-base">{car.status === 'Disponible' ? 'Libre' : 'Rentado'}</p>
          </div>
        </div>
      </div>
    </div>
